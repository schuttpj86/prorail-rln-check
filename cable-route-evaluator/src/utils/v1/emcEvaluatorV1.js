...
async function computeTechnicalRoomDistance(routeGeometry, routeRd, layer) {
  if (!layer) {
    console.warn("❌ Technical rooms layer is null/undefined");
    return null;
  }
  
  if (typeof layer.createQuery !== "function") {
    console.warn("❌ Technical rooms layer does not have createQuery function. Layer type:", layer.type, "Layer ID:", layer.id);
    return null;
  }

  try {
    console.log("🔍 Querying technical rooms layer:", layer.title || layer.id);
    ...
    console.log("   Buffer distance:", bufferDistance, "meters");
    const result = await layer.queryFeatures(query);
    console.log(`   Found ${result?.features?.length || 0} technical room features`);
    
    const geometries = (result?.features || []).map((feature) => feature.geometry).filter(Boolean);
    const distance = computeMinimumDistance(routeRd, geometries);
    
    if (distance !== null) {
      console.log(`   ✅ Minimum distance to technical room: ${distance.toFixed(2)}m`);
    } else {
      console.log("   ℹ️ No technical rooms found within buffer");
    }
    
    return distance;
  } catch (error) {
    console.error("❌ Failed to query technical rooms:", error.message);
    console.error("   Layer URL:", layer.url);
    console.error("   Layer loaded:", layer.loaded);
